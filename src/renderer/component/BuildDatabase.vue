    ...
    <div>
        ...
        <div v-if="'data' === currentTab">
            ...
            <card dis-hover :bordered="false" class="margin-top-16">
                ...
                <pre style="overflow: auto"><template v-for="table in tables.full">mysqldump -h{{data.options.originIP}} -P{{data.options.originPort}} -u{{data.options.originUser}} -p{{data.options.originPassword}} --set-gtid-purged=OFF --default-character-set=utf8mb4 mymonitor --single-transaction --tables {{table.name}} | mysql -h{{data.options.destIP}} -P{{data.options.destPort}} -u{{data.options.destUser}} -p{{data.options.destPassword}} --default-character-set=utf8mb4 mymonitor <br/></template></pre>
                <pre style="overflow: auto"><template v-for="table in tables.part">mysqldump -h{{data.options.originIP}} -P{{data.options.originPort}} -u{{data.options.originUser}} -p{{data.options.originPassword}} --set-gtid-purged=OFF --default-character-set=utf8mb4 mymonitor --single-transaction --tables {{table.name}} -w"{{table.where}}" | mysql -h{{data.options.destIP}} -P{{data.options.destPort}} -u{{data.options.destUser}} -p{{data.options.destPassword}} --default-character-set=utf8mb4 mymonitor <br/></template></pre>
